...
validate_contract() {
  ...
  require_text "Windows script bootstraps Zig GNU tools" "${windows_script}" 'Ensure-Zig-GNU-Build-Environment'
  require_text "Windows script downloads Zig" "${windows_script}" 'ziglang.org/download'
  require_text "Windows script uses Zig linker wrapper" "${windows_script}" 'CARGO_TARGET_X86_64_PC_WINDOWS_GNU_LINKER'
