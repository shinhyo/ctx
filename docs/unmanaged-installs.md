...
Tools that install ctx from GitHub Releases should point at this URL pattern:
...
Stable GitHub releases are expected to contain exactly these ctx-managed assets:
...
Do not publish per-platform `.sha256` sidecars when `SHA256SUMS` is present.

When Homebrew is offered as an unmanaged install path, update
`ctxrs/homebrew-tap` to the same release version and verify the formula before
