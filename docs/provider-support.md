...
| Claude | `local_import_when_supported` | `~/.claude/projects` or an explicit Claude projects JSONL tree. |
| OpenCode | `local_import_when_supported` | `~/.local/share/opencode/opencode.db` or an explicit OpenCode SQLite DB. |
| Antigravity | `detected_unsupported` | Native import is blocked until a stable local transcript path/schema is proven. |
| Gemini | `local_import_when_supported` | `~/.gemini` or an explicit Gemini CLI history tree. |
| Cursor | `detected_unsupported` | Native import is blocked until persisted local DB/files and a read-only parser are proven. |
| Copilot CLI | `local_import_when_supported` | `~/.copilot/session-state` or an explicit Copilot CLI session-state tree. |
| Factory AI Droid | `local_import_when_supported` | `~/.factory/sessions` or an explicit Factory AI Droid sessions tree. |
| Amp | `detected_unsupported` | Native local thread import is blocked because no stable local thread file path/schema is proven. |
