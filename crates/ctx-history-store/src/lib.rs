...
mod catalog_tests {
    ...
    fn provider_check_constraints_accept_search_only_providers() {
        ...
        let source_count: i64 = store
            .conn
            .query_row(
                "SELECT COUNT(*) FROM capture_sources WHERE provider IN ('kilo', 'crush', 'goose', 'dexto', 'copilot_cli', 'factory_ai_droid', 'continue', 'openhands', 'qwen_code', 'kimi_code_cli', 'autohand_code', 'kiro_cli', 'codebuddy', 'zed', 'custom')",
                [],
                |row| row.get(0),
            )
            .unwrap();
        let catalog_count: i64 = store
            .conn
            .query_row(
                "SELECT COUNT(*) FROM catalog_sessions WHERE provider IN ('kilo', 'crush', 'goose', 'dexto', 'copilot_cli', 'factory_ai_droid', 'continue', 'openhands', 'qwen_code', 'kimi_code_cli', 'autohand_code', 'kiro_cli', 'codebuddy', 'zed', 'custom')",
                [],
                |row| row.get(0),
            )
            .unwrap();
        assert_eq!(source_count, 15);
        assert_eq!(catalog_count, 15);
    }
